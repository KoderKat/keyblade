...
class DeviantPipeline(object):
  def __init__(self):
    ...
    self.connection = sqlite.connect(BASE_DIR + 'deviantart.db')
    self.cursor = self.connection.cursor()
    self.cursor.execute('CREATE TABLE IF NOT EXISTS deviantart (id INTEGER PRIMARY KEY, fandom VARCHAR(100), ' \
  ...
  def process_item(self, item, spider):
    self.cursor.execute("SELECT * FROM deviantart WHERE url_title=? AND url_author=?", (item['url_title'], item['url_author']))
    result = self.cursor.fetchone()
    if result:
      #log.msg("Item already in database: %s" % item, level=log.DEBUG)
      print ("MESSAGE: Item already in database: %s" % item)
    else:
      self.cursor.execute("""INSERT INTO deviantart (fandom, url_fandom, title, url_title, author, url_author, genre, author_rating, wordcount, numchapters) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                         (
                           item['fandom'],
                           item['url_fandom'],
                           item['title'],
